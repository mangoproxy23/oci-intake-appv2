...
model or assistant, and no machine-readable trailer linking an assistant session
...
Removing an assistant attribution later requires rewriting commit messages and
force-pushing the branch, which changes every affected commit hash. It is cheaper not to
add one.
